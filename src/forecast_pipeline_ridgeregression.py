...
MODEL_NAME = os.getenv("MODEL_NAME", "Ridge")
...
def get_top_features_from_coefficients(feature_cols, coefficient_arrays, top_n=5):
    """
    For Ridge, feature ranking uses absolute coefficient size after scaling.
...
def get_ridge_coefficients(return_model):
    """
    Pulls coefficients from the fitted Ridge pipeline.
    Because we use StandardScaler, coefficients are comparable across features.
    """
    ridge_model = return_model.named_steps["ridge"]
    ...
    for symbol in SYMBOLS:
        ...
        for test_date in test_dates:
            ...
            for horizon in HORIZONS:
                ...
                coefficient_arrays.append(get_ridge_coefficients(return_model))
